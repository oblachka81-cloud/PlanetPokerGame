...
io.on('connection', (socket) => {
    ...
    // ========== ИСПРАВЛЕННЫЙ table:ready ==========
    socket.on('table:ready', ({ tableId, userId }) => {
        const table = tables[tableId];
        if (!table) return;
        
        // Привязываем новый socketId к игроку по userId
        ...
        // Обновляем socketId и присоединяем к комнате
        ...
        // Отправляем текущее состояние игры этому игроку
        const cfg = TABLES[tableId];
