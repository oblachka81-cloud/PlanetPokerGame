...
io.on('connection', (socket) => {
    ...
    // ========== table:ready ==========
    socket.on('table:ready', ({ tableId, userId }) => {
        console.log(`[table:ready] tableId=${tableId} userId=${userId} socketId=${socket.id}`);

        const table = tables[tableId];
        if (!table) {
            console.log(`[table:ready] Стол ${tableId} не найден (null)`);
            return;
        }

        // Отменяем таймаут удаления если игрок успел переподключиться
        const key = `${tableId}:${userId}`;
        if (reconnectTimeouts[key]) {
            clearTimeout(reconnectTimeouts[key]);
            delete reconnectTimeouts[key];
            console.log(`[${tableId}] Таймаут переподключения отменён для userId=${userId}`);
        }

        const player = table.players.find(p => String(p.telegramId) === String(userId));
        if (!player) {
            console.log(`[table:ready] Игрок userId=${userId} не найден за столом ${tableId}`);
            return;
        }
        ...
        console.log(`[${tableId}] ${player.name} переподключился, отправляем game:round`);
...
function leaveAllTables(socketId) {
    Object.keys(tables).forEach(tableId => {
        if (!tables[tableId]) return;
        const player = tables[tableId].players.find(p => p.socketId === socketId);
        if (!player) return;

        // Даём 10 секунд на переподключение перед удалением
        const key = `${tableId}:${player.telegramId}`;
        if (reconnectTimeouts[key]) clearTimeout(reconnectTimeouts[key]);

        reconnectTimeouts[key] = setTimeout(() => {
            if (!tables[tableId]) return;
            const before = tables[tableId].players.length;
            tables[tableId].players = tables[tableId].players.filter(p => p.telegramId !== player.telegramId);
            if (tables[tableId].players.length !== before) {
                console.log(`[${tableId}] ${player.name} не переподключился, удаляем. Осталось: ${tables[tableId].players.length}`);
                io.to(tableId).emit('table:playerLeft', {
                    tableId,
                    players: tables[tableId].players.map(p => ({
                        userId: p.telegramId,
